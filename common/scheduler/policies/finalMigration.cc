#include "finalMigration.h"
#include <iomanip>
using namespace std;
FinalMigration::FinalMigration(const PerformanceCounters *performanceCounters,
    ...
    : performanceCounters(performanceCounters),
      ...
      std::vector<int> FinalMigration::map(String taskName, int taskCoreRequirement,
...
std::vector<migration> FinalMigration::migrate(
    ...
    const std::vector<bool> &activeCores)
{
    ...
    for (int c = 0; c < coreRows * coreColumns; c++)
    {
        if (activeCores.at(c))
        {
            ...
            if (temperature > worryTemperature && performanceCounters->getFreqOfCore(c) < (maxFrequency / 2))
            {
...
int FinalMigration::getColdestCore(const std::vector<bool> &availableCores)
{
    int coldestCore = -1;
...
void FinalMigration::logTemperatures(const std::vector<bool> &availableCores)
{
...
float FinalMigration::averageTemperature(int core) {
